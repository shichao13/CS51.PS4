...
struct
  ...
  let rec lookup (d: dict) (k: key) : value option =
    ...
    (* For the two case, we choose which branch to go down depending on if ky is big or small*)
    | Two (d1, (ky, va), d2) ->
  ...
  let rec val_compare (a: int list) (c: int) : bool =
    ...
    | [] -> true
    | x :: xs ->
      if x = c then val_compare xs c else false

  let rec internal_loop (d: dict) (a: int list) (level: int) : int list =
    match d with
    (* leaf only occurs with empty tree *)
    | Leaf -> []

    (* If we are at the bottom, record the level *)
    | Two (Leaf, _, Leaf) -> level :: a
